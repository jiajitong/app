...
void func_bt_message(u16 msg)
{
    // int klu_flag = 0;
     u8 led_bt_cbt_tbl11[] = {
    0x00, 0x55, 0x0a, 0x00, 1, 0x06, 0xc0, 0x03, 0x06, 1, 0x06, 0xc0, 0x03, 0x06, 1, 0x00, 0x00, 0x00, 0x00, 1,
};
    ...
    //    if (func_cb.sta == FUNC_BT_DUT)
    //     {
    //        memcpy(&xcfg_cb.led_btinit, led_bt_cbt_tbl11, sizeof(led_bt_cbt_tbl11));    //改变灯效
    //        func_cb.sta = FUNC_BT;
    //     }
        break;
    ...
    case KD_PLAY_PWR_USER_DEF:
     if (bt_is_connected())
        ...
        //  }else if ((func_cb.sta == FUNC_BT_DUT))
        // {
        //    memcpy(&xcfg_cb.led_btinit, led_bt_cbt_tbl11, sizeof(led_bt_cbt_tbl11));    //改变灯效
        //    func_cb.sta = FUNC_BT;
        }
    ...
    case KU_SIRI:   //单击打开低延时
        // sys_set_eq();
         if (!bt_is_low_latency())
        {
            bt_low_latency_enble();
            func_bt_mp3_res_play(RES_BUF_GAME_MODE_MP3, RES_LEN_GAME_MODE_MP3);
        }
        else
        {
            bt_low_latency_disable();
            func_bt_mp3_res_play(RES_BUF_MUSIC_MODE_MP3, RES_LEN_MUSIC_MODE_MP3);
        }
        break;
