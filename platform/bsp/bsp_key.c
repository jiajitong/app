    ...
    while (1)
    {
        ...
        if ((bsp_key_pwr_scan() & K_PWR_MASK) == K_PWR)
        {
            ...
            if (restart_chk_en)
            {
                ...
                {
                    if (tick_check_expire(ticks, pwron_press_nms))
                    { //长按开机时间配置
...
static void power_on_check_do(void)
{
    ...
    if ((rtccon9 & BIT(4)) && (xcfg_cb.ch_out_auto_pwron_en) && (xcfg_cb.ch_box_type_sel != 3))
    { //charge inbox wakeup直接开机(5V完全掉电无维持电压的仓不开机)
...
#if CHARGE_EN
    //是否需要拿起开机
    if ((LVDCON & BIT(17)) || (rtccon9 & BIT(3)) || (ch_dc_sw_rst) || (sys_cb.inbox_wken_flag))
    { //vusb reset or wakeup
    ...
    while (1)
    {
        ...
        {
            if (tick_check_expire(ticks, 5000))
                gui_en = 0;
            else
                gui_en = 1;
            ...
            if ((!pwrkey_pressed_flag) && (chbox_sta))
            {
                ...
                if (!sys_cb.rtc_first_pwron && !tick_check_expire(ticks, 5000) && sys_cb.vbat > 2950)
                    continue;
...
{
    u8 step = 0;
    if (s_abs((int)knob_level[*key_val] - (int)adc_val) > 4)
    { //取绝对值消抖
...
u16 key_knob_process2(u16 *key_val)
{
    ...
    if (*key_val != KEY_VOL_DOWN_PREV && *key_val != KEY_VOL_UP_NEXT)
    { //过滤掉其他按键
        key_flag = 0;
        if (*key_val == NO_KEY)
        {
