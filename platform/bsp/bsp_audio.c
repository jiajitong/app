...
void ttp_sdadc_process(u8 *ptr, u32 samples, int ch_mode);
...
#elif !BT_AEC_EN && !BT_NLMS_AEC_EN && (BT_SNDP_EN || BT_SNDP_DMIC_EN || BT_SCO_AINS2_EN)
    ...
    #if BT_SNDP_DMIC_EN
    uint8_t cfg_sdadc_soft_gain_en = 2;           //sdadc drc 双声道
    #else
    uint8_t cfg_sdadc_soft_gain_en = 1;           //sdadc drc 单声道
    #endif
