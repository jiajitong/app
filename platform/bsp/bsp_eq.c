...
void set_enc_channel(u8 ch);

bool mic_eq_close_en;
void mic_eq_disable_en(bool flag)
{
    if(flag == 0){
        mic_eq_close_en = 0;
        //printf("open_eq\n");
...
void bsp_smic_parse_cmd(void)
{
    ...
    if(ctlcmd == 'N'){                                  //smic close
        mic_eq_disable_en(1);                           //close eq
        set_enc_channel(channel);
        bsp_enc_dbg_ack(channel, 0);                    //sucess
    }else if(ctlcmd == 'Y'){                            //smic open
        mic_eq_disable_en(0);                           //open eq
        set_enc_channel('3');
        bsp_enc_dbg_ack(channel, 0);                    //sucess
    }else{
...
#if DMIC_DBG_EN && (BT_SNDP_DMIC_EN || BT_SCO_BCNS_EN || BT_SCO_DMNS_EN)
u32 getcfg_enc_ch_status(void);
u32 getcfg_enc_ch_mic(void);
u16 getcfg_bt_ch_gain(void);
u32 getcfg_bt_ch_mic(void);
u16 getcfg_enc_ch_gain(void);
...
void bsp_anc_parse_cmd(void)
{
    ...
    do {
        ...
        if (xcfg_cb.anc_mode == 1) {        //Hybrid mode
            ...
            if (bit_ctl & BIT(4)) {
                bsp_anc_set_mic_gain(0, 1, ptr[10]);//默认值是0,然后adjust
