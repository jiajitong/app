...
static u32 bsp_charge_box_process_do(void)
{
...
#endif
    if ((bsp_charge_box_sta() == 2) || (vhouse_sta == 2))
    { //是否充满
        bsp_charge_box_exit();
        unlock_code_charge();
        bsp_charge_inbox_wakeup_enable(); //INBOX Wakeup enable
    ...
    {   //DC IN online?
        ...
        { //dc_in = 0, 且inbox = 0
            bsp_charge_leakage_set(1, 0);
            if ((chbox_cb.out2pwr_en) || (vhouse_sta))
            { //拿起开机
                sys_cb.outbox_pwron_flag = 1;
                return 2;
            }
    ...
    {
        ledseg_displaybat(IN_CHARGE);
        chbox_cb.dc_in = 1;
        inbox_cb.off_cnt = 0;
        inbox_cb.on_cnt = 0;
        bsp_charge_leakage_set(1, 0);
        if ((chbox_cb.counter % 1000) == 0)
        { //5秒
...
void bsp_charge_box_enter(u32 out_auto_pwron)
{
    ...
    if ((chbox_cb.out2pwr_en) && (!xcfg_cb.ch_box_type_sel))
    {                                                                 //短暂掉0V的仓
